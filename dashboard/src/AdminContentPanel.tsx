...
import type { AdminCredentials, ListItem, NewsPost } from './types';

/**
 * Espace superadmin pour piloter le contenu du site vitrine (actualités,
 * pays couverts, réseaux de paiement) sans jamais repasser par le code —
 * le site public lit ces mêmes données via /site-content/* (voir Landing.tsx).
 */
export function AdminContentPanel({ credentials }: { credentials: AdminCredentials }) {
  const [section, setSection] = useState<'news' | 'countries' | 'networks'>('news');
